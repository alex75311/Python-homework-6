...
class Worker:
    def __init__(self, name='', surname='', position=''):
        self.__income = {'wage': 10000, 'bonus': 1000}
        self.name = name
...
class Position(Worker):
    def get_full_name(self):
        print(f'Уважаемый, {self.name} {self.surname}')

    def get_total_income(self):
        total = 0
        for el in (self._Worker__income).values():
            total += el
        print(f'Доход с учетом премии составляет {total}')


a = Position('Вася', 'Пупкин')
a.get_full_name()
a.get_total_income()
